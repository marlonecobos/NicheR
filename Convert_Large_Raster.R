Convert_Large_Raster <- function(raster_stack, 
                               output_name = "raster_db",
                               chunk_height = 500,
                               keep_sql = FALSE) {
  
  # Create database connection
  sqlite_file <- paste0(output_name, ".sqlite")
  rdata_file <- paste0(output_name, ".RData")
  ...
  # Save as RData
  message("Saving RData file: ", rdata_file)
  save(env_df, file = rdata_file)
  
  # Delete SQL file if requested
  if (!keep_sql) {
    message("Deleting SQLite file: ", sqlite_file)
    file.remove(sqlite_file)
  } else {
    message("SQLite file kept: ", sqlite_file)
  }
  
  message("Processing complete. Data frame has ", nrow(env_df), " rows and ", ncol(env_df), " columns")
